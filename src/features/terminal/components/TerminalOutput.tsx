        ...
        <div key={index} className="mb-2">
          ...
          {Array.isArray(cmd.output) ? (
            cmd.output.map((line, lineIndex) => (
              <div key={lineIndex} className={cmd.isError ? 'text-red-400' : getLineColor(line)}>
                {makeContactClickable(line)}
              </div>
            ))
          ) : (
            <div className={cmd.isError ? 'text-red-400' : getLineColor(cmd.output as string)}>
              {cmd.output}
            </div>
          )}
          {cmd.projectList ? (
          ...
          {index === commandHistory.length - 1 && showShutdownSequence && <ShutdownSequence />}
        </div>
      ))}
    </div>
  );
